 ...
 *     frames
 */
function createProcessedMediaStream(sourceStream1, sourceStream2, sourceStream3, transform, signal) {
  // For this sample, we're only dealing with video tracks.
  /** @type {!MediaStreamTrack} */
  let sourceTrack1, sourceTrack2, sourceTrack3;
  sourceTrack1 = sourceStream1.getVideoTracks()[0];
  if(sourceStream2){
    sourceTrack2 = sourceStream2.getVideoTracks()[0];
  }
  if(sourceStream3){
    sourceTrack3 = sourceStream3.getVideoTracks()[0];
  }

  const processedTrack =
      createProcessedMediaStreamTrack(sourceTrack1, sourceTrack2, sourceTrack3, transform, signal);
...
class FrameTransform { // eslint-disable-line no-unused-vars
   ...
   */
  async transform(frame1, frame2, frame3, controller) {}
  /** Frees any resources used by this object. */
  destroy() {}
}
...
class Pipeline { // eslint-disable-line no-unused-vars
  ...
  /** @return {?MediaStreamSource, ?MediaStreamSource, ?MediaStreamSource} */
  getSource() {
    return [this.source1_, this.source2_, this.source3_];
  }
   ...
   */
  async updateSource(mediaStreamSource1, mediaStreamSource2, mediaStreamSource3) {
    if (this.source1_) {
      this.abortController_.abort();
      this.abortController_ = new AbortController();
      this.source1_.destroy();
      this.processedStream_ = null;
    }
    if (this.source2_) {
      this.source2_.destroy();
    }
    if (this.source3_) {
      this.source3_.destroy();
    }

    this.source1_ = mediaStreamSource1;
    this.source2_ = mediaStreamSource2;
    this.source3_ = mediaStreamSource3;
    this.source1_.setDebugPath('debug.pipeline.source1_');
    if(this.source2_){
      this.source2_.setDebugPath('debug.pipeline.source2_');
    }
    if(this.source3_){
      this.source3_.setDebugPath('debug.pipeline.source3_');
    }
    console.log(
        '[Pipeline] Updated sources.',
        'debug.pipeline.source_ = ', this.source1_, this.source2_, this.source3_);
    await this.maybeStartPipeline_();
  }

  /** @private */
  async maybeStartPipeline_() {
    // if (this.processedStream_ || !this.source1_  || !this.source2_ || !this.frameTransform_ ||
    //     !this.sink_) {
    //   return;
    // }
    let sourceStream1, sourceStream2, sourceStream3;
    sourceStream1 = await this.source1_.getMediaStream('../../../video/chrome.webm');
    if(this.source2_){
    ...
    try {
      this.processedStream_ = createProcessedMediaStream(
          sourceStream1, sourceStream2, sourceStream3, async (frame1, frame2, frame3, controller) => {
            if (this.frameTransform_) {
              await this.frameTransform_.transform(frame1, frame2, frame3, controller);
            }
  ...
  destroy() {
    ...
    this.abortController_.abort();
    if (this.source1_) this.source1_.destroy();
    if (this.source2_) this.source2_.destroy();
    if (this.source3_) this.source3_.destroy();
    if (this.frameTransform_) this.frameTransform_.destroy();
    if (this.sink_) this.sink_.destroy();
  }
}
