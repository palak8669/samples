...
class CanvasSource { // eslint-disable-line no-unused-vars
  ...
  updateCanvasVisibility() {
    if (this.canvas_.parentNode && !this.visibility_) {
      ...
      const inputVideoContainer =
          document.getElementById('inputVideoContainer');
          inputVideoContainer.appendChild(this.canvas_);
          // inputVideoContainer.parentNode.insertBefore(
          // this.canvas_, inputVideoContainer);
    }
  }
  /** @private */
  ...
  animate(now) {
    ...
    if (this.visibility_) {
      ...
      const inputVideoContainer =
          document.getElementById('inputVideoContainer');
      const inputVideo = inputVideoContainer.firstElementChild;
      if (inputVideo) {
        this.canvas_.width = roundToEven(inputVideo.clientWidth);
      }
    }
    this.canvas_.width = 480;
    this.canvas_.height = 270;
